...
var urls = {
  ...
  chrome: '%s/%s/%s/chromedriver-%s.zip',
...
const sys = require('./getSystem')();
const basePath = path.join(__dirname, '../', 'selenium', process.env.SYSTEM);
...
function computeDownloadUrls(opts, askedOpts) {
  ...
  const fsPaths = {
    selenium: path.join(
      basePath,
      'server',
      downloadUrls.selenium.replace(opts.seleniumBaseURL, ''),
    ),
  };
  if (opts.drivers.chrome) {
    ...
    const chromeDriverArchitecture = getChromeDriverArchitecture(opts.drivers.chrome.version, opts.drivers.chrome.arch); 
    downloadUrls.chrome = util.format(
      urls.chrome,
      opts.drivers.chrome.baseURL,
      opts.drivers.chrome.version,
      chromeDriverArchitecture,
      chromeDriverArchitecture,
    );

    // to keep the path same as original after download from https://edgedl.me.gvt1.com/edgedl/chrome/chrome-for-testing, just add the following workaround
    let originChromeDriverArchitecture = chromeDriverArchitecture === 'mac-x64' ? 'mac64' : chromeDriverArchitecture;
    const originalChromeDownloadUrl = util.format(
      '%s/%s/chromedriver_%s.zip',
      opts.drivers.chrome.baseURL,
      opts.drivers.chrome.version,
      originChromeDriverArchitecture,
    ); 
    fsPaths.chrome = path.join(
      basePath,
      'chrome',
      originalChromeDownloadUrl.replace(opts.drivers.chrome.baseURL, ''),
    );
  }
  if (opts.drivers.ie) {
    ...
    fsPaths.ie = path.join(basePath, 'ie', downloadUrls.ie.replace(opts.drivers.ie.baseURL, ''));
  }
  if (opts.drivers.firefox) {
    ...
    fsPaths.firefox = path.join(
      basePath,
      'firefox',
      downloadUrls.firefox.replace(opts.drivers.firefox.baseURL, ''),
    );
  }
  if (opts.drivers.edge) {
    downloadUrls.edge = getEdgeDriverUrl(opts.drivers.edge.version);
    fsPaths.edge = path.join(
      basePath,
      'edgedriver',
      downloadUrls.edge.substring(downloadUrls.edge.lastIndexOf('/') + 1),
    );
  }
  ...
  if (opts.drivers.chromiumedge) {
    downloadUrls.chromiumedge =
      ...
      fsPaths.chromiumedge = path.join(
        basePath,
        'chromiumedge',
        downloadUrls.chromiumedge.replace(opts.drivers.chromiumedge.baseURL, ''),
      );
  }


  return [downloadUrls, fsPaths];
}
...
function getFirefoxDriverVersionForUrl(version) {
  if (compareVersions(version, '0.9.0') >= 0) {
    return 'v' + version;
  } else if (sys === 'win32' && ['0.5.0', '0.7.1', '0.8.0'].indexOf(version) !== -1) {
    return 'v' + version;
  }
  return version;
}
...
function getFirefoxDriverArchitecture(version, wantedArchitecture) {
  ...
  switch (sys) {
    ...
    default:
      throw new Error('No Firefox driver is available for platform "' + sys + '"');
  }
}
