...
var debug = require('debug')('neocore-selenium-drivers:download');
var fs = require('fs');
var os = require('os');
...
function install(opts, cb) {
  ...
  logger('----------');
  logger('neocore-selenium-drivers installation starting');
  logger('----------');
  logger('');

  var [urls, fsPaths] = computeDownloadUrls({
  ...
  if (fsPaths.chrome) {
    tasks.push(setDriverFilePermissions.bind(null, fsPaths.chrome));
  }

  if (fsPaths.firefox) {
    tasks.push(setDriverFilePermissions.bind(null, fsPaths.firefox));
  }

  if (fsPaths.chromiumedge) {
    tasks.push(setDriverFilePermissions.bind(null, fsPaths.chromiumedge));
  }
  ...
  function download(opts, cb) {
    ...
    if (opts.fsPaths.chrome) {
      installers.push({
        ...
        to: opts.fsPaths.chrome,
      });
    }

    if (sys === 'win32' && opts.fsPaths.ie) {
      installers.push({
    ...
    if (sys === 'win32' && opts.fsPaths.edge) {
      installers.push({
        installer: installEdgeDr,
        from: opts.urls.edge,
        to: opts.fsPaths.edge,
      });
    }
  ...
  function uncompressGzippedFile(from, gzipFilePath, cb) {
    ...
    var writeStream = fs.createWriteStream(extractPath).once('error', function(error) {
      cb.bind(null, new Error('Could not write to ' + extractPath));
    });
    var gunzippedContent = fs
    ...
    if (from.substr(-7) === '.tar.gz') {
      ...
      gunzippedContent.pipe(writeStream).on('finish', function() {
        cb();
      });
    }
  }
...
function asyncLogEnd(logger, cb) {
  setImmediate(function() {
    ...
    logger('');
    logger('-----');
    logger('neocore-selenium-drivers installation finished');
    logger('-----');
    cb();
  });
}
...
function createDirs(paths, cb) {
  var installDirectories = Object.keys(paths).map(function(name) {
    return paths[name];
  });
